...
const session = sign({ email, since: Date.now() })!;
is("a valid session is read back", verify(session)?.email === email);
is("a forged session is refused", verify(session.split(".")[0] + ".forged") === null);
is("a session older than 30 days is refused", verify(sign({ email, since: Date.now() - 31 * 24 * 3600 * 1000 })!) === null);
is("a cookie written before accounts were simplified still reads", verify(sign({ email, since: Date.now(), plan: "basic" } as never)!)?.email === email);
is("extra fields in an old cookie are not carried forward", !("plan" in (verify(sign({ email, since: Date.now(), plan: "basic" } as never)!) ?? {})));
