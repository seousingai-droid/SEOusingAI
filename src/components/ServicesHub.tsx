...
const nodes = [
  { label: "Audit and fixes", x: 400, y: 70 },
  { label: "Strategy", x: 672, y: 215 },
  { label: "Content", x: 585, y: 440 },
  { label: "Authority", x: 215, y: 440 },
  { label: "Reporting", x: 128, y: 215 },
];
export default function ServicesHub({ className = "" }: { className?: string }) {
  return (
    <svg viewBox="0 0 800 520" role="img" aria-label="Five service lines, audit and fixes, strategy, content, authority, and reporting, all connected to human review at the center" className={className}>
      ...
      {nodes.map((n) => (
        <g key={n.label}>
          ...
          <text x={n.x - 70} y={n.y + 8} fill="#e9edf9" fontSize="23" fontWeight="600" fontFamily="var(--font-display), sans-serif">{n.label}</text>
        </g>
      ))}
