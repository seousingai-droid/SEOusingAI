...
import { readKey } from "@/lib/license";
...
export async function POST(req: Request) {
  ...
  const tier = readKey(key);
  if (!tier) return NextResponse.json({ ok: false, error: "That key is not valid. Check for typos, or paste it exactly as it appears in your purchase email." }, { status: 400 });
  ...
  const token = sign({ email, plan: tier, since: Date.now(), sites: existing?.sites });
  if (!token) return NextResponse.json({ ok: false, error: "Sign-in is not set up on this server yet." }, { status: 503 });
  const res = NextResponse.json({ ok: true, session: { email, plan: tier } });
